/* ══════════════════════════════════════════════════════════════
   WarEra+ — Political View: party.js come modulo ES (Fase 2, Stage 7)
   ------------------------------------------------------------------
   Conversione diretta. TomSelect da CDN a import npm.

   _lastMinVotesToWin: nell'originale scriveva SIA la bare var SIA
   window._lastMinVotesToWin nella stessa riga (già sincronizzate in
   pratica). Qui una sola variabile (config.js: minVotesToWin/
   setMinVotesToWin, Stage 2) — organizer.js (creato subito dopo in
   questo stage) legge la stessa.
   ══════════════════════════════════════════════════════════════ */
...
import {
  escapeHtml, APP_BASE,
  currentCountryId, countryNamesMap,
  currentPartyId, setCurrentPartyId,
  setCurrentPartyData,
  setLastCongressElection,
  setMinVotesToWin,
} from './config.js';
...
export async function loadPartiesForSelector() {
  ...
  if (refreshBtn) {
    refreshBtn.onclick = async () => {
      ...
      // Bust cache for party + user calls — namespace 'pol' (Stage 3),
      // era 'we_/party_...' nell'originale, ora 'we_pol_/party_...'.
      const key = 'we_pol_/party_' + JSON.stringify({ id: currentPartyId });
      try { localStorage.removeItem(key); } catch (_) {}
      await loadPartyDetails(currentPartyId);
...
/* ── PARTY DETAILS ── */
export async function loadPartyDetails(partyId) {
  ...
  try {
    ...
    const electionsDataPromise = localFetch('/elections', { countryId: currentCountryId });
    ...
    document.getElementById('partyOverview').innerHTML = `
      ...
      <div class="party-overview-item"><span class="party-overview-label">${t('party_country')}</span><span>${countryNamesMap.get(partyData.country) || partyData.country || currentCountryId}</span></div>
    ...
    const lastCongressElection = congressElections[0];
    setLastCongressElection(lastCongressElection);

    if (lastCongressElection) {
      const electionData = await localFetch('/election', { id: lastCongressElection._id });
      ...
      setMinVotesToWin(minVotesToWin);
