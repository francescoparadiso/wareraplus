...
import { openNationsView, isNationsViewOpen, closeNationsView } from './nationsOverlay.js';
import { openGuideView, isGuideViewOpen, closeGuideView } from './guideOverlay.js';
import { openPrivateView, closePrivateView, isPrivateViewOpen } from './privateOverlay.js';
...
/* WarEra+ (segnalato dall'utente): il logo era decorativo, e in una app
   dove ogni sezione si apre a schermo intero la prima cosa che si prova
   per "tornare a casa" e' cliccarci sopra. Ora lo fa: chiude la sezione
   aperta, chiude il pannello e riporta la mappa alla vista pulita — lo
   stesso resetView() che la barra mobile ha da sempre. */
function buildBrand() {
  const brand = el('button', 'wp-mb-brand', { type: 'button', title: 'WarEra+' });
...
/* Torna alla mappa nuda: chiude la sotto-vista, spegne la selezione e
   chiude il pannello. E' il gemello desktop di mobileMenuBar.js:resetView. */
function goHome() {
  closeAnySubview();
  ...
  trackEvent('go-home', { source: 'brand' });
}

/* ---- Bottone "Mappa" (solo dentro una sotto-vista) ----------------
   WarEra+ (segnalato dall'utente): si chiamava "Indietro" ma non e' mai
   stato un indietro — chiude la sezione aperta e riporta alla mappa,
   qualunque strada si sia fatta dentro la sezione. Chi lo leggeva si
   aspettava di risalire di un passo e si ritrovava a casa. Il
   comportamento resta identico (e' quello giusto per una sezione a
   schermo intero); cambia il nome, che ora dice dove porta. */
function buildBackButton() {
  const btn = el('button', 'wp-mb-btn wp-mb-back', { type: 'button' });
  btn.appendChild(iconEl('map'));
  btn.appendChild(regSpan('home'));
...
function closeAnySubview() {
  ...
  if (isPrivateViewOpen()) closePrivateView();
...
function renderFavorites(panel) {
  ...
  if (mu.length) {
    panel.appendChild(el('div', 'wp-mb-res-group', { text: mbT('groupMus') }));
    mu.forEach(id => {
      // Il dato vivo della directory se c'è, altrimenti i metadati
      // salvati al momento del pin (vedi src/app/pins.js: le MU non
      // stanno in `state`, a differenza di nazioni e alleanze).
...
function buildInsightsDropdown() {
  ...
  // Statistiche nazioni (src/nations/*): la gemella di Statistiche
  // alleanze un livello più in basso — panoramica, 1 vs 2 e scheda della
  // singola nazione con i suoi cittadini.
  const natItem = el('button', 'wp-mb-item', { type: 'button' });
  natItem.appendChild(iconEl('globe'));
  natItem.appendChild(regSpan('nationStats'));
  natItem.addEventListener('click', () => {
    closeAnySubview();
    openNationsView();
    trackEvent('menubar-open-nations');
    closeAllDropdowns();
  });
  panel.appendChild(natItem);
  ...
  // "Battaglie" della barra (quello accende/spegne i marker sulla mappa):
  // qui si guarda il passato, lì il presente.
  const btlItem = el('button', 'wp-mb-item', { type: 'button' });
  btlItem.appendChild(iconEl('swords'));
  btlItem.appendChild(regSpan('battleArchive'));
  btlItem.addEventListener('click', () => {
    closeAnySubview();
    openBattlesView();
    trackEvent('menubar-open-battles');
    closeAllDropdowns();
  });
  panel.appendChild(btlItem);

  // News: il notiziario completo (vedi src/app/newsView.js) — tutte le
  // notizie che il ticker in cima alla mappa mostra solo a campione.
  const newsItem = el('button', 'wp-mb-item', { type: 'button' });
  newsItem.appendChild(iconEl('newspaper'));
  newsItem.appendChild(regSpan('news'));
  newsItem.addEventListener('click', () => {
    closeAnySubview();
    openNewsView();
    trackEvent('menubar-open-news');
    closeAllDropdowns();
  });
  panel.appendChild(newsItem);

  // Unità Militari: elenco/ricerca di tutte le MU, schede e classifiche
  // (vedi src/mu/*). Stesso pattern delle altre voci: chiude quello che
  // c'è aperto, poi apre il proprio overlay.
  const muItem = el('button', 'wp-mb-item', { type: 'button' });
  muItem.appendChild(iconEl('users'));
  muItem.appendChild(regSpan('muExplorer'));
  muItem.addEventListener('click', () => {
    closeAnySubview();
    openMuView();
    trackEvent('menubar-open-mu');
    closeAllDropdowns();
  });
  panel.appendChild(muItem);

  // Area riservata (src/private/*): la voce resta visibile a TUTTI, anche
  // a chi non entrera' mai. Nasconderla a chi non e' loggato vorrebbe dire
  // che nessuno scopre che esiste; chi ci clicca legge cos'e' e chiude.
  const privateItem = el('button', 'wp-mb-item', { type: 'button' });
  privateItem.appendChild(iconEl('lock'));
  privateItem.appendChild(regSpan('privateArea'));
  privateItem.addEventListener('click', () => {
    closeAnySubview();
    openPrivateView();
    trackEvent('menubar-open-private');
    closeAllDropdowns();
  });
  panel.appendChild(privateItem);

  // Guida "Come si usa" (src/guide/*): ultima voce del menù, dopo le
  // sezioni che spiega.
  const guideItem = el('button', 'wp-mb-item', { type: 'button' });
  guideItem.appendChild(iconEl('help'));
  guideItem.appendChild(regSpan('howTo'));
  guideItem.addEventListener('click', () => {
    closeAnySubview();
    openGuideView();
    closeAllDropdowns();
  });
  panel.appendChild(guideItem);
...
function renderResults(query) {
  ...
  const mus = (getCachedDirectory() || []).filter(m => m.name?.toLowerCase().includes(q)).sort(byRelevance).slice(0, 6);
  ...
  if (mus.length) {
    searchResults.appendChild(el('div', 'wp-mb-res-group', { text: mbT('groupMus') }));
...
function makeMuRow(m) {
  ...
  img.onerror = () => { img.style.visibility = 'hidden'; };
  row.appendChild(img);
  row.appendChild(el('span', 'wp-mb-fav-name', { text: m.name }));
  row.appendChild(pinToggle('mu', m._id, { name: m.name, avatarUrl: m.avatarUrl, country: m.country }));
  row.addEventListener('click', () => {
    closeAnySubview();
    openMuView(m._id);
    trackEvent('search-mu', { muId: m._id, source: 'menubar' });
    closeSearch(m.name);
  });
...
function selectNation(n) {
  ...
  const label = state.labelsData?.find(l => l.properties?.countryId === n._id);
  if (label && state.map) state.map.flyTo({ center: label.coordinates, zoom: Math.max(state.map.getZoom(), 3) });
  // WarEra+ (segnalato dall'utente): cercare una nazione tingeva la mappa ma
  // lasciava chiuso il pannello di destra, che fino a ieri si apriva solo
  // col click sul territorio — due gesti per una cosa sola. Cercare E'
  // gia' una scelta esplicita, quindi apre il pannello come il click. Lo
  // faceva gia' la ricerca mobile (mobileMenuBar.js: selectNation) e i
  // Preferiti qui sopra (goToDetail): era questo il percorso rimasto fuori.
...
function enterDesktop() {
  ...
  // NB: il credit autore NON viene più rilocato qui. Su desktop c'è già la
  // pill #wp-author-pill in basso al centro (index.html), sempre visibile:
  // ripeterlo in fondo a Impostazioni era una doppia attribuzione. Su
  // mobile la pill è nascosta (shell.css, @media max-width:768px), quindi
  // lì il credit resta nel drawer — vedi mobileMenuBar.js:enterMobile.
}
