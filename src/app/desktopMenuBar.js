...
import { openAdminView, closeAdminView, isAdminViewOpen, sembraAdmin } from './adminOverlay.js';
...
function closeAnySubview() {
  ...
  if (isAdminViewOpen()) closeAdminView();
...
function buildInsightsDropdown() {
  ...
  // Amministrazione (src/private/adminView.js): la voce compare SOLO a
  // chi risulta amministratore all'ultimo accesso da questo browser. E'
  // un suggerimento d'interfaccia, non un permesso — il server rifiuta
  // comunque chi non lo e' — e serve a non mostrare a tutti una porta
  // che si apre per due persone.
  if (sembraAdmin()) {
    const adminItem = el('button', 'wp-mb-item', { type: 'button' });
    adminItem.appendChild(iconEl('lock'));
    adminItem.appendChild(regSpan('adminSection'));
    adminItem.addEventListener('click', () => {
      closeAnySubview();
      openAdminView();
      trackEvent('menubar-open-admin');
      closeAllDropdowns();
    });
