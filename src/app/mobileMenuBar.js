...
function buildInsightsAccordion() {
  ...
  // Battaglie (src/battles/*) — stessa voce del menu desktop.
  const btl = el('button', 'wp-mmb-item', { type: 'button' });
  btl.appendChild(iconEl('swords'));
  btl.appendChild(regSpan('battleArchive'));
  btl.addEventListener('click', () => {
    closeAnySubview();
    openBattlesView();
    trackEvent('menubar-open-battles', { source: 'mobile' });
    closeDrawer();
  });
  body.appendChild(btl);
  ...
  // Area riservata (src/private/*) — visibile a tutti, vedi la nota
  // gemella in desktopMenuBar.js.
  const privateItem = el('button', 'wp-mmb-item', { type: 'button' });
  ...
  body.appendChild(privateItem);

  // Amministrazione — vedi la nota gemella in desktopMenuBar.js.
  if (sembraAdmin()) {
    const adminItem = el('button', 'wp-mmb-item', { type: 'button' });
