...
async function loadElectionsHistory() {
  try {
    ...
    // WarEra+ hook (additivo, unica riga): segnala che _electionHistory
...
// non può chiamare SenateView.open() o leggere _currentCongressElectionId
// direttamente da fuori l'iframe — sono dichiarate con const/let, che in
// JS NON diventano proprietà di `window` (solo var/function lo fanno),
// quindi `contentWindow.SenateView` è sempre undefined dall'esterno anche
// se `SenateView` esiste. Soluzione: un evento in ingresso ascoltato QUI,
// dove il riferimento per nome (non tramite window.) è valido.
window.addEventListener('wareraplus:open-senate', () => {
  if (_currentCongressElectionId) {
    loadElection(_currentCongressElectionId).then(() => SenateView.open());
  } else {
